...
def save_file(df,name):
	...
	if file.is_file():
		with open(p+name+'.csv', 'a', encoding="utf-8") as f:
...
def radio_to_vec(name):
	...
	for df in  pd.read_csv(path+name+'.csv', chunksize=chunk, index_col=0, low_memory=False):
		...
		d1 = d1.melt(id_vars = ['txn','location','position','dx_type','icd10'], value_name = 'report')
		d1 = d1.sort_values(['txn', 'icd10', 'variable'], ascending=True)
		d1 = d1[d1['variable'] != 'report']
		d1 = d1[(d1['report'] != '') & (d1['report'])]
		df = d1[['txn','location','position','report','dx_type','icd10']]
		df = pd.concat([df.drop('icd10', axis=1), df['icd10'].map(icd10_map)], axis=1)
		#print(df)
		for c in df.columns:
			if c != 'txn' and c != 'icd10':
		...
		df = df[['txn','location','position','dx_type','report_gram_1','report_gram_2','report_gram_3','report_gram_4','report_gram_5','icd10']]
		df = df.fillna(0)
		n = n + chunk
		print("Converted "+name+' '+str(n))
		save_file(df,name)
